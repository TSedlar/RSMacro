...
public class Test extends Macro implements Renderable, PixelListener {
    ...
    private long idFor(int slot, int tolerance) {
        PixelModel model = Inventory.createModelAt(slot, tolerance);
        if (model == null) {
            return -1;
        }
        int id = model.pixels.length;
        long rgbs = 0;
        int length = 0;
        for (Pixel pixel : model.pixels) {
            rgbs += (pixel.color.getRed() + pixel.color.getGreen() + pixel.color.getBlue());
            length++;
        }
        id += (rgbs / (length * 3));
        return id;
    ...
    public void render(Graphics2D g) {
        ...
        for (int i = 0; i < ids.length; i++) {
            ...
            String text = Long.toString(ids[i]);
